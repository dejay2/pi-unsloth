...
test("DFlash plus n-gram cache produces one conflict-free server command", () => {
	...
	assert.deepEqual(buildLoadPayload("ggml-org/Qwen3.8-27B-GGUF:Q4_K_M", settings), {
...
test("DFlash can use a file already on the Unsloth computer", () => {
	const payload = buildLoadPayload("org/model:Q4", {
		load: {
			speculation: {
				draft: { kind: "dflash", source: { kind: "file", path: "C:\\models\\draft.gguf" }, depth: 7 },
				ngram: { kind: "none" },
			},
		},
	});
...
test("buildModelOverridePayload maps per-model server settings", () => {
	assert.deepEqual(buildModelOverridePayload("org/model:Q8_0", {
...
test("findConfiguredModelId matches Unsloth's unqualified active model to its saved quant", () => {
	...
	assert.equal(findConfiguredModelId(provider, "org/other:Q8_0"), "org/other:Q8_0");
	assert.equal(findConfiguredModelId(provider, "missing"), undefined);
});

test("startupModelId prefers the saved default and falls back to a configured current model", () => {
	const provider = {
		baseUrl: "http://x/v1",
		apiKey: "k",
		defaultModelId: "org/default:Q4",
		models: { "org/default:Q4": {}, "org/current:Q8": {} },
	};
	assert.equal(startupModelId(provider, "org/current:Q8"), "org/default:Q4");
	assert.equal(startupModelId({ ...provider, defaultModelId: "missing" }, "org/current:Q8"), "org/current:Q8");
	assert.equal(startupModelId({ ...provider, defaultModelId: undefined }, "missing"), undefined);
});
