...
export type NgramSpeculation =
	...
	| { kind: "simple" | "map-k" | "map-k4v"; sizeN?: number; sizeM?: number; minHits?: number };
...
export interface UnslothProviderConfig {
	...
	apiKey: string;
	/** Model restored when pi starts and remembered by Unsloth Studio. */
...
export function findConfiguredModelId(
	...
	return Object.keys(provider.models).find((modelId) => splitModelRef(modelId).modelPath === activeModelId);
}

export function startupModelId(provider: UnslothProviderConfig | undefined, currentModelId?: string): string | undefined {
	if (!provider) return undefined;
	if (provider.defaultModelId && provider.defaultModelId in provider.models) return provider.defaultModelId;
	if (currentModelId && currentModelId in provider.models) return currentModelId;
	return undefined;
}

export function setDefaultModel(data: UnslothConfig, providerId: string, modelId: string): boolean {
...
const MANAGED_SPEC_FLAGS = new Set([
	"--spec-type",
	"--spec-draft-model", "--model-draft", "-md",
	"--spec-draft-hf", "--hf-repo-draft", "-hfd", "-hfrd",
	"--spec-draft-n-max",
...
function removeManagedSpecArgs(args: string[] | undefined): string[] {
	...
	for (let i = 0; i < (args?.length ?? 0); i++) {
		...
		if (arg && [...MANAGED_SPEC_FLAGS].some((flag) => arg.startsWith(`${flag}=`))) continue;
		if (arg !== undefined) kept.push(arg);
	}
	return kept;
}

function appendNumberArg(args: string[], flag: string, value: number | undefined): void {
...
function buildStructuredSpeculation(speculation: SpeculativeSettings): {
	...
	} else if (ngram.kind === "simple" || ngram.kind === "map-k" || ngram.kind === "map-k4v") {
...
export function buildLoadPayload(modelId: string, s: UnslothModelSettings): Record<string, unknown> {
	...
	if (resolved.speculativeType) payload.speculative_type = resolved.speculativeType;
	...
	if (resolved.extraArgs.length > 0) payload.llama_extra_args = resolved.extraArgs;
	return payload;
}

/** Body for Unsloth's saved per-model load override. The model id stays qualified by its quant. */
export function buildModelOverridePayload(modelId: string, s: UnslothModelSettings): Record<string, unknown> {
	const load = s.load ?? {};
	const resolved = resolvedLoadSettings(load);
